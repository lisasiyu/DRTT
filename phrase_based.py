...
if __name__ == "__main__":
    import doctest
    doctest.testmod()
    with open(src,'r',encoding='utf-8') as f1, open(tgt,'r',encoding='utf-8') as f2, open(align,'r',encoding='utf-8') as f3:
        for s, t, a in zip(f1,f2,f3):
            s = s.strip()
            t = t.strip()
            a = a.strip().split()
            align_ = []
            for item in a:
                x,y = item.split('-')
                align_.append((int(x),int(y)))
            phrases = phrase_extraction(s, t, align_)
            out = []
            for i in sorted(phrases):
                source_i, source_j = int(i[0][0]),int(i[0][1])
                target_i, target_j = int(i[1][0]),int(i[1][1])
                start = source_i
                end = source_j
                s_len = end - start
                j = (start,end)
