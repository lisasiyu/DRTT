...
class GeneratorHubInterface(nn.Module):
    ...
    def translate(
        self, sentences: List[str], n_best: int = 1, beam: int = 5, verbose: bool = False, **kwargs
    ) -> List[str]:
        return self.sample(sentences, n_best, beam, verbose, **kwargs)

    def sample(
        self, sentences: List[str], n_best: int = 1, beam: int = 1, verbose: bool = False, **kwargs
    ) -> List[str]:
        if isinstance(sentences, str):
            return self.sample([sentences], beam=beam, verbose=verbose, **kwargs)[0]
        tokenized_sentences = [self.encode(sentence) for sentence in sentences]
        batched_hypos = self.generate(tokenized_sentences, beam, verbose, **kwargs)
        return [[self.decode(hypos[i]["tokens"]) for i in range(n_best)] for hypos in batched_hypos]
    def translate_one(
        self, sentences: List[str], n_best: int = 1, beam: int = 5, verbose: bool = False, **kwargs
    ) -> List[str]:
        return self.sample_one(sentences, n_best, beam, verbose, **kwargs)

    def sample_one(
        self, sentences: List[str], n_best: int = 1, beam: int = 1, verbose: bool = False, **kwargs
    ) -> List[str]:
        if isinstance(sentences, str):
            return self.sample_one([sentences], n_best=n_best, beam=beam, verbose=verbose, **kwargs)[0]
        tokenized_sentences = [self.encode(sentence) for sentence in sentences]
        batched_hypos = self.generate(tokenized_sentences, beam, verbose, **kwargs)
